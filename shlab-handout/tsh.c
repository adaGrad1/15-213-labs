...
void waitfg(pid_t pid)
{
    //todo proper checking of system call output
    int status;
...
void sigchld_handler(int sig) 
{
    ...
    if (errno != ECHILD && errno != 0) unix_error("waitpid error");
    errno = olderrno;
}
...
void sigtstp_handler(int sig) 
{
    ...
    sigprocmask(SIG_BLOCK, &mask_all, &prev_all);
    pid_t pid = fgpid(jobs);
    if(pid != 0 && getjobpid(jobs, pid)->state != ST){
        killpg(pid, sig);
    }
    sigprocmask(SIG_SETMASK, &prev_all, NULL);
}
...
void listjobs(struct job_t *jobs) 
{
	...
	if (jobs[i].pid != 0) {
        /*int status;
        pid_t return_pid = waitpid(jobs[i].pid, &status, WNOHANG & WUNTRACED);
        if (return_pid == 0 && WIFSTOPPED(status)) {
            printf("STOPPED: ");
        }*/
...
int Setpgid(pid_t pid, pid_t pgid){
    ...
    if(res < 0) unix_error("failed setpgid: ");
    return res;
}